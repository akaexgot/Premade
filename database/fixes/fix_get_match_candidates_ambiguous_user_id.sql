-- Corrige "column reference user_id is ambiguous" en get_match_candidates.
...
begin
  ...
  select
    ...
    coalesce(
      array_remove(array_agg(distinct g.title), null),
      array[]::text[]
    ) as games,
    u.bio,
    u.is_online
    ...
    and not exists (
      ...
      where m.is_active = true
        and (
          ...
          or
          (m.user_id_1 = u.id and m.user_id_2 = v_profile_id)
        )
    )
    and not exists (
      select 1
...
alter function public.get_match_candidates(uuid, varchar, uuid, integer)
  owner to postgres;
